...
class LocationsControllerTests(ControllerTestBase):
    ...
    def test_get_ev_locations(self):
        """
        This API Product provides the list of all public Shell Recharge locations. The
        list includes Shell Recharge network and locations available publicly through
        our roaming partners. The end point provides flexible search criteria in order
        to get the list of Shell Recharge Network. The end point provides the details
        such as the exact location/address of the site along with the up-to-date
        status information of all the charging units in the site. Supported Search
        Options* Based on status of the Charging units. Eg : Available or Occupied*
        Based on available connector types.* Based on minimum Power output (in kW)
        available* Based on a specific charging unit ID (EVSE ID) .
        """
        ...
        self.controller.get_ev_locations(
            request_id,
            evse_status,
            connector_types,
            connector_min_power,
